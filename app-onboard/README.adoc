...
TIP: Openshift Local allows to spin up a cluster using Openshift, https://www.okd.io/[OKD], https://microshift.io/[MicroShift] or even http://podman.io/[Podman] - just use `crc config set preset okd; crc setup; crc start` for an OKD cluster.
...
----
mvn clean package oc:build oc:resource oc:apply -Popenshift -Djkube.build.switchToDeployment=true
----

NOTE: OpenShift officially deprecates the usage of DeploymentConfigs in version 4.14 (see https://access.redhat.com/articles/7041372). To force `jkube` to use +Deployments+ instead of +DeploymentConfigs+ the `-Djkube.build.switchToDeployment=true` switch is used.
